    ...
    showModal(modalDialog(
      title = div(
        ...
        div(
          style = "display: inline-block; float: right;",
          a(
            "Tutorial", target = "_blank", href = "popup-ncats-SEQUIN-tutorial.html#ipsc-profiler", 
...
# LoadData server function. reactiveValues object 'd' is returned and values 
# are used in main server function.
LoadData <- function(input, output, session, maxSamples = Inf) {
    ...
    showModal(modalDialog(
      title = div(
        ...
        div(
          style = "display: inline-block; float: right;",
          a(
            "Tutorial", target = "_blank", href = "popup-ncats-SEQUIN-tutorial.html#selection-of-data",
            style = "color: black !important;"
          )
        )
      ),
  ...
  user_made_meta_table <- reactive({
    if(is.null(load_datasets_table()) || nrow(load_datasets_table()) == 0 ||
       is.null(d$select_datasets_table_rows_selected) ||
       length(d$select_datasets_table_rows_selected) == 0) return()
    if(getOption("standalone")) {
  ...
  observeEvent(input$save_load_selected, {
    ...
    # Exit if either no data selected or user-uploaded data is incomplete
    if(is.null(d$select_datasets_table_rows_selected) && !d$end_ready) return()
    if(d$end_partial) return()
    ...
    withProgress(message = loadMessage, value = 0, {
      ...
      datasets <- nSamples <- selectedDatasetsTable <- NULL
      if(!is.null(d$select_datasets_table_rows_selected)) {
        datasets <- load_datasets_table()$experiment_name[d$select_datasets_table_rows_selected]
      }
      if(input$data_type == "Single-cell" && input$downsample_select) {
        nSamples <- min(input$downsample_nCells, maxSamples)
      }
      
      subsetMetaList <- list()
      for(dataset in names(d$datasetsRowsSelected)) {
        myRows <- d$datasetsRowsSelected[[dataset]]
        myCols <- d$inputFactors[[dataset]]
        subsetMetaList[[dataset]] <- d$selectSamplesTableList[[dataset]][myRows, myCols, drop = F]
      }
      
      datasetList <- CreateDataList(
        datasets = datasets,
        dataType = input$data_type,
        userDataCounts = count_content(),
        userDataMeta = meta_content(),
        userDataName = input$caption,
        nSamples = nSamples,
        seed = input$downsample_seed,
        subsetMetaList = subsetMetaList
      )
      
      if(input$data_type == "Bulk") {
        dataMerged <- MergeBulk(datasetList = datasetList)
      } else {
        dataMerged <- MergeSeurat(seuratList = datasetList)
      }      
  ...
  observeEvent(input$load_data, {
    ...
    withProgress(message = loadMessage, value = 0, {
      ...
      datasets <- nSamples <- selectedDatasetsTable <- NULL
      if(!is.null(d$select_datasets_table_rows_selected)) {
        selectedDatasetsTable <- load_datasets_table()[d$select_datasets_table_rows_selected, , drop = F]
        datasets <- selectedDatasetsTable$experiment_name
      }
      if(input$data_type == "Single-cell" && input$downsample_select) {
        nSamples <- min(input$downsample_nCells, maxSamples)
      }
      ...
      userMadeMetaDF <- NULL
      if(input$data_type == "Single-cell" && !is.null(user_made_meta_table()) && 
        !is.null(input$user_made_meta_rows_selected)) {
        ...
        if(!is.null(userMadeMetaDF)) {
          dataMerged <- AddUserMadeMeta(
            dataList = dataMerged,
            userMadeMeta = userMadeMetaDF
          )
        }
      }
      
      if(input$data_type == "Single-cell" && "seurat_clusters" %in% colnames(dataMerged$metadata)) {
